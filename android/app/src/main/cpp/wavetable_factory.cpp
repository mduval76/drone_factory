...
namespace {
    ...
    std::vector<float> generateTriangleWavetable() {
        ...
        for (auto k = 1; k <= HARMONICS_COUNT; ++k) {
            for (auto j = 0; j < WAVETABLE_LENGTH; ++j) {
                ...
                triangleWavetable[j] += 8.f / std::pow(PI, 2.f) * std::pow(-1.f, k) * std::pow(2 * k - 1, -2.f) * std::sin((2.f * k - 1.f) * phase);
            }
        }
    ...
    std::vector<float> generateSquareWavetable() {
        ...
        for (auto k = 1; k <= HARMONICS_COUNT; ++k) {
            for (auto j = 0; j < WAVETABLE_LENGTH; ++j) {
                const auto phase = 2.f * PI * 1.f * j / WAVETABLE_LENGTH;
                squareWavetable[j] += 4.f / PI * std::pow(2.f * k - 1.f, -1.f) * std::sin((2.f * k - 1.f) * phase);
            }
        }
...
std::vector<float> WavetableFactory::sineWavetable() {
  return generateWavetableOnce(m_sineWavetable, &generateSineWavetable);
}

std::vector<float> WavetableFactory::triangleWavetable() {
  return generateWavetableOnce(m_triangleWavetable, &generateTriangleWavetable);
}

std::vector<float> WavetableFactory::squareWavetable() {
  return generateWavetableOnce(m_squareWavetable, &generateSquareWavetable);
}

std::vector<float> WavetableFactory::sawtoothWavetable() {
  return generateWavetableOnce(m_sawtoothWavetable, &generateSawWavetable);
}
} 
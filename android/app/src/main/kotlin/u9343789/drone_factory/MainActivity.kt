    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        flutterEngine?.let {
            ...
            when (call.method) {
                ...
                "setIsSoloed" -> {
                    val trackId = call.argument<Int>("trackId") ?: 0
                    val isSoloed = call.argument<Boolean>("isSoloed") ?: false
                    CoroutineScope(Dispatchers.Main).launch {
                        try {
                            nativeSynthesizer.setIsSoloed(trackId, isSoloed)
                            result.success(null)
                        } 
                        catch (e: Exception) {
                            result.error("ERROR", "Failed to set solo for track $trackId", e.localizedMessage)
                        }
                    }
                }

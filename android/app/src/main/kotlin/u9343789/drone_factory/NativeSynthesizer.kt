...
class NativeSynthesizer : Synthesizer {
    ...
    private external fun setIsSoloed(synthesizerHandle: Long, trackId: Int, isSoloed: Boolean)
    ...
    override suspend fun setIsSoloed(trackId: Int, isSoloed: Boolean) = withContext(Dispatchers.Default) {
        synchronized(synthesizerMutex) {
            createNativeHandleIfNotExists()
            setIsSoloed(synthesizerHandle, trackId, isSoloed)
        }
    }
